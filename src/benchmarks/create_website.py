...
from ann_benchmarks.plotting.utils import (
	compute_all_metrics, create_linestyles, create_pointset, get_plot_label
)
import plot
...
def convert_linestyle(ls):
	...
	for algo in ls.keys():
		algostyle = ls[algo]
		new_ls[algo] = (
			convert_color(algostyle[0]), convert_color(algostyle[1]),
...
def create_plot(
	all_data, xn, yn, linestyle, j2_env, dataset_name: str, additional_label="", plottype="line"
):
	...
	latex_code = j2_env.get_template("latex.template"
			...
			plot_data=plot_data, caption=get_plot_label(xm, ym),
			dataset_name=dataset_name, xlabel=xm["description"], ylabel=ym["description"]
	...
	return j2_env.get_template("chartjs.template"
			...
			args=args, latex_code=latex_code, button_label=button_label,
			data_points=plot_data, xlabel=xm["description"], ylabel=ym["description"],
...
def build_index_site(datasets, algorithms, j2_env, _):
